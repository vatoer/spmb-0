...
import { FAQ } from "@/modules/beranda/data/faq";
import Link from "next/link";

const Faq = () => {
  return (
    <div className="w-full py-8 px-8 md:px-8 bg-zinc-100  flex flex-col gap-2 items-center justify-center">
        ...
        <Accordion type="single" collapsible>
          {FAQ.map((faq, index) => (
            <AccordionItem key={index} value={`item-${index}`}>
              <AccordionTrigger>{faq.pertanyaan}</AccordionTrigger>
